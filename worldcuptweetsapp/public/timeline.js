...
function drawInfo(p) {
    ...
    p.append('text')
        .classed('sum', true)
        .attr('x', 5)
        .text(function(d) { return d.sum.formatMoney() + ' RUR'; });
    ...
    p.append('text')
        .attr('x', 5)
        .text(function(d) { return d3.min(d.donates, function(d) { return d.sum; }).formatMoney() + ' RUR'; })
        .attr('y', 28);
    ...
    p.append('text')
        .attr('x', 5)
        .text(function(d) { return d3.max(d.donates, function(d) { return d.sum; }).formatMoney() + ' RUR'; })
        .attr('y', 40);
}
